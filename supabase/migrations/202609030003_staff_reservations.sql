...
begin
  ...
  with points(t) as (
    ...
    where r.reservation_date = p_reservation_date and r.status <> 'CANCELLED' and r.id <> result_id
  ...
  select coalesce(max(guests + coalesce((select sum(r.adults+r.children+r.infants)::integer
    from public.reservations r where r.reservation_date=p_reservation_date and r.status<>'CANCELLED'
      and r.id<>result_id and r.start_time<=points.t
  ...
  if p_parking and exists (select 1 from public.reservations r
    where r.reservation_date=p_reservation_date and r.status<>'CANCELLED' and r.id<>result_id and r.parking
      and r.start_time<p_end_time and r.start_time+make_interval(mins=>r.duration_minutes)>p_start_time)
  ...
  values (result_id,p_reservation_date,p_start_time,p_duration_minutes,p_adults,p_children,p_infants,p_parking,trim(p_customer_name),trim(p_phone),nullif(trim(p_email),''),nullif(trim(p_note),''),p_source,'CONFIRMED',auth.uid())
