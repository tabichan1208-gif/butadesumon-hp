...
function MediaLibrary({media}:{media:AdminMedia[]}){
  ...
  const upload=(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();if(selectedFiles.length===0){setNotice("写真を選択してください。");return}const form=e.currentTarget;const data=new FormData();selectedFiles.forEach(file=>data.append("images",file));startTransition(async()=>{const result=await uploadLibraryImage(data);setNotice(result.message);if(result.ok){form.reset();setSelectedFiles([]);router.refresh()}})};
