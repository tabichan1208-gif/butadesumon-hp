...
function MediaLibrary({media}:{media:AdminMedia[]}){
  const router=useRouter();const[pending,startTransition]=useTransition();const[notice,setNotice]=useState("");
  const fileInput=useRef<HTMLInputElement>(null);const[dragging,setDragging]=useState(false);const[fileCount,setFileCount]=useState(0);
  ...
  const drop=(e:DragEvent<HTMLLabelElement>)=>{e.preventDefault();setDragging(false);const files=Array.from(e.dataTransfer.files).filter(file=>file.type.startsWith("image/")).slice(0,20);if(files.length===0)return;const transfer=new DataTransfer();files.forEach(file=>transfer.items.add(file));if(fileInput.current)fileInput.current.files=transfer.files;setFileCount(files.length)};
  const upload=(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const form=e.currentTarget;const data=new FormData(form);startTransition(async()=>{const result=await uploadLibraryImage(data);setNotice(result.message);if(result.ok){form.reset();setFileCount(0);router.refresh()}})};
  return <div className="cms-stack">{notice&&<p className="admin-notice">{notice}</p>}<section className="admin-panel"><div className="panel-head"><div><h2>画像ライブラリ</h2><p>登録済み画像をメイン写真や店舗紹介写真に使えます</p></div></div><form className="library-upload" onSubmit={upload}><label className={`image-drop-zone${dragging?" dragging":""}`} htmlFor="library-image" onDragEnter={e=>{e.preventDefault();setDragging(true)}} onDragOver={e=>e.preventDefault()} onDragLeave={()=>setDragging(false)} onDrop={drop}><span>複数の写真をここへドラッグ＆ドロップ</span><small>{fileCount?`${fileCount}枚を選択中`:"またはクリックして写真を選択（最大20枚）"}</small><input ref={fileInput} id="library-image" name="images" type="file" accept="image/jpeg,image/png,image/webp" multiple required onChange={e=>setFileCount(e.target.files?.length??0)}/></label><button className="button" disabled={pending}>{pending?"追加中…":fileCount?`${fileCount}枚を追加`:"画像を追加"}</button></form><div className="media-grid">{media.length===0?<p className="empty-state">画像はまだありません。</p>:media.map(item=><article key={item.id}><div className="media-photo" style={{backgroundImage:`url(${publicImageUrl(item.storage_path)})`}}/><p>{item.alt_text||"登録画像"}</p><form onSubmit={run(chooseSiteImage)}><input type="hidden" name="path" value={item.storage_path}/><select name="slot" defaultValue="hero"><option value="hero">メイン写真</option><option value="about">店舗紹介写真</option></select><button className="button secondary" disabled={pending}>この画像を使う</button></form></article>)}</div></section></div>;
}
