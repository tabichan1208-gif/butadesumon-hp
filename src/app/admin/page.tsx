...
export default async function AdminPage() {
  ...
  const[{data},{data:settingsData},{data:contentData},{data:pigData},{data:faqData},{data:mediaData},interiorResult,emailResult]=await Promise.all([
  ...
  return <AdminDashboard emailSettings={{...defaultEmailSettings,...(emailResult.data??{})}} interior={interiorResult.data??[]} interiorError={Boolean(interiorResult.error)} reservations={reservations} settings={settings} copy={copy} pigs={pigData??[]} faqs={faqData??[]} media={(mediaData??[]).filter(item=>!item.deleted_at)}/>;
}
