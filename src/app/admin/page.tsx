...
export default async function AdminPage() {
  ...
  const[{data},{data:settingsData},{data:contentData},{data:pigData},{data:faqData},{data:mediaData},interiorResult]=await Promise.all([
  ...
  return <AdminDashboard interior={interiorResult.data??[]} interiorError={Boolean(interiorResult.error)} reservations={reservations} settings={settings} copy={copy} pigs={pigData??[]} faqs={faqData??[]} media={(mediaData??[]).filter(item=>!item.deleted_at)}/>;
}
